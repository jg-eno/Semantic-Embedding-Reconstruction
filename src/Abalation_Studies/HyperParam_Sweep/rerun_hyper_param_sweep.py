...
PARAGRAPH_DIM   = 1024
MAX_TEXT_LEN    = 128
BATCH_SIZE      = 32      # lowered from 64 to fix OOM on the high-hidden_dim / high-layer reruns
GRAD_ACCUM      = 4       # doubled to keep the effective batch size (BATCH_SIZE * GRAD_ACCUM) at 128
LEARNING_RATE   = 3e-4
...
def run_trial():
    ...
    # NOTE: when running the targeted OOM-rerun sweep (sweep_config_failed_reruns.yaml),
    # the three hyperparams arrive as one composite dict under cfg.hparam_combo instead
    # of as flat top-level keys, since the failed configs aren't a clean cross-product.
    # Fall back to flat keys so this script still works unmodified with the original
    # full-grid sweep_config.yaml.
    if "hparam_combo" in cfg:
        combo              = cfg.hparam_combo
        prefix_len         = combo["prefix_len"]
    ...
    # Materialize the fixed 20k-sample subset ONCE per trial.
    # Previously this called load_dataset(..., streaming=True) fresh inside the
    # epoch loop below — i.e. NUM_EPOCHS cold HTTP streams per trial, all hitting
    # the same handful of remote parquet shards. That repeated network exposure
    # is what caused the 'read operation timed out' / retry storms during long
    # runs.
    #
    # Fix: stream + .take(TOTAL_SAMPLES) ONCE (bounded — does NOT download the
    # full 1M-record dataset), shuffle that capped stream, then convert to a
    # local in-memory Dataset via Dataset.from_list(). Every epoch after that
    # iterates the local copy — zero network calls.
    from datasets import Dataset as HFDataset

    print(f"Pulling {TOTAL_SAMPLES} samples from {DATASET} (one-time, bounded stream)...")
    MAX_LOAD_RETRIES = 3
    local_dataset = None
    for attempt in range(1, MAX_LOAD_RETRIES + 1):
        try:
            bounded_stream = load_dataset(DATASET, split="train", streaming=True).shuffle(
                seed=42, buffer_size=10_000
            )
            local_dataset = HFDataset.from_list(
                [s for s in tqdm(bounded_stream.take(TOTAL_SAMPLES), total=TOTAL_SAMPLES, desc="Caching subset")]
            )
            break
        except Exception as e:
            print(f"  Data pull attempt {attempt}/{MAX_LOAD_RETRIES} failed: {e}")
            if attempt == MAX_LOAD_RETRIES:
                raise
    print(f"Cached {len(local_dataset)} samples locally. No further network access needed this trial.")
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        # Re-shuffle the already-local data each epoch (no network call — this
        # just reorders an in-memory Arrow table).
        epoch_data = local_dataset.shuffle(seed=epoch)
        progress_bar = tqdm(epoch_data, desc=f"Epoch {epoch}", total=TOTAL_SAMPLES, leave=False)
